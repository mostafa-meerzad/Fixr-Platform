...
export default function MediaScreen() {
  ...
  useEffect(() => {
    jobsService.get(jobId).then(({ data }) => {
      const media: Array<{ id: string; url: string; type: string }> =
        (data as any).media ?? [];
      const existingPhotos = media
        .filter((m) => m.type !== 'video')
        .slice(0, 3)
        .map((m) => ({ uri: m.url, mediaId: m.id, status: 'done' as const }));
      const existingVideo = media.find((m) => m.type === 'video');
      if (existingPhotos.length > 0) setPhotos(existingPhotos);
      if (existingVideo) setVideo({ uri: existingVideo.url, mediaId: existingVideo.id, status: 'done' });
    }).catch(() => {});
  }, [jobId]);
  ...
  async function handleAddPhoto() {
    ...
    setPhotos((prev) => [
      ...prev,
      { uri: asset.uri, mediaId: tempId, status: "uploading" },
    ]);

    try {
      const uploaded = await mediaService.uploadJobMedia(
        jobId,
        asset.uri,
        mimeType,
      );
      setPhotos((prev) =>
        prev.map((p) =>
          p.mediaId === tempId
            ? { ...p, mediaId: uploaded.id, status: "done" }
            : p,
        ),
      );
    } catch {
      ...
      toast.show({
        message: t("homeowner.post.photoUploadError"),
        variant: "error",
      });
    }
  }
  ...
  async function handleAddVideo() {
    ...
    try {
      ...
      setVideo((prev) =>
        prev ? { ...prev, mediaId: uploaded.id, status: "done" } : null,
      );
    } catch {
      setVideo(null);
      toast.show({
      ...
      >
        ...
        <View style={styles.photoRow}>
          {[0, 1, 2].map((i) => {
              ...
              return (
                <View key={i} style={styles.photoSlot}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      handleRemovePhoto(photo.mediaId, photo.status)
                    }
            ...
            return (
              <View key={i} style={[styles.photoSlot, styles.photoSlotGhost]} />
            );
          })}
        </View>

        {/* ── Video ── */}
        <Text style={[styles.sectionLabel, { marginTop: Spacing.s5 }]}>
          {t("homeowner.post.videoLabel")}
        </Text>
